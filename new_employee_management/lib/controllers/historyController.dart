...
import 'package:employee_management/data/models/month_wise_history.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';

class HistoryController extends BaseApiController {
  RxList<MonthWiseHistoryModelData> monthWiseData =
      <MonthWiseHistoryModelData>[].obs;
  RxBool monthWiseDataAvailable = false.obs;
  ...
  Future getMonthWiseData({startDay, endDay}) async {
    try {
      var response = await getDio()!.get(EndPoints.attendencesPath,
          queryParameters: {'start_date': startDay, 'end_date': endDay});
      MonthWiseHistoryModel monthData =
          MonthWiseHistoryModel.fromJson(response.data);
      if (monthData.success) {
        monthWiseData.clear();
        monthWiseData.addAll(monthData.data);
        monthWiseDataAvailable.value = true;
        update();
        return monthData;
      }
    } on DioException catch (e) {
